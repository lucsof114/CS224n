...
class NMT(nn.Module):
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for v in torch.split(Y,1): 
            Y_t = torch.squeeze(v,0)
            Ybar_t = torch.cat((o_prev,Y_t), axis = 1) 
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(self.att_projection(enc_hiddens), torch.unsqueeze(dec_hidden, 2))
        ...
        a_t = torch.bmm(torch.transpose(enc_hiddens,2,1), torch.unsqueeze(alpha_t,2))
        U_t = torch.cat((dec_hidden, torch.squeeze(a_t,2) ), axis = 1)
